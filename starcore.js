...
mobile: MethodMobile, 
browser: opcion == '1' ? ['MikuBot-MD', 'Edge', '20.0.04'] : methodCodeQR ? ['MikuBot-MD', 'Edge', '20.0.04'] : ["Ubuntu", "Chrome", "20.0.04"],
auth: {
creds: state.creds,
...
console.log('🍭 MikuBot-MD no tiene Sub-Bots vinculados.');
return;
}
...
console.log(chalk.bold.greenBright(`🍭 MikuBot conectado correctamente.`))

await connectSubBots();
} catch (error) {
console.error(chalk.bold.cyanBright(`🥀 Error al iniciar MikuBot-MD: `, error))
}
})();
...
if (existsSync(dir)) conn.logger.info(` SE ACTUALIZO - '${filename}' CON ÉXITO`)
else {
conn.logger.warn(`SE ELIMINO UN ARCHIVO : '${filename}'`)
return delete global.plugins[filename];
}
} else conn.logger.info(`SE DETECTO UN NUEVO PLUGIN : '${filename}'`)
...
conn.logger.error(`HAY UN ERROR REQUIERE EL PLUGIN '${filename}\n${format(e)}'`);
} finally {
...
const directories = ['./Sessiona/', './JadiBot/']
